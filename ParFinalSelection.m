...
arcs = [1,1; 2,1; 2,2; 2,5; 2,10;2,20; 3,1];
method = ["DSS", "LazyHV"];
...
%             if ~(ismember(problems(b), ["MinusDTLZ3", "MinusDTLZ4"]))
%                 continue;
%             end
            if objs(c)==3
                pop = 91;
            ...
            for arcsiz = [1,2,5,10,20,50,100,200,500,1000,2000]*pop
                ...
                for a=1:length(algos)
                    for d=1:21   
                        ...
                        if (~exist(sprintf('%s/Result/%s',folder, name),'dir'))
                            mkdir(sprintf('%s/Result/%s/',folder, name))
                        end
                        n=n+1;
                        task(n).algo = algos(a);
                        ...
                        task(n).savePath=sprintf('%s/Result/%s/%s.mat',folder,name,name2);
                    end
                end
                parfor i = 1:63
                    index = i;
...
function selection(seq, result, PF, path)
    ...
    if runtime>3600
        ...
        hv = HV(normalizedRes,1.2);
        save(path,"runtime","memory","res","hv","truncationTime", "removalTime","selectionTime");
    end
end
